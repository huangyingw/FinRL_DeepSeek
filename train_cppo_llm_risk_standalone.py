...
# 超参数：自动从 ParamStore (Redis) 读取（Optuna 优化结果）
def load_best_params():
    """从 ParamStore Redis 后端加载 finrl_deepseek 命名空间下的最佳超参数"""
    try:
        from pkg.params import get_namespace
        params = get_namespace('finrl_deepseek')
        if params:
            print(f"✅ Loaded {len(params)} best params from ParamStore (finrl_deepseek)")
            return params
        print("⚠️ ParamStore 中无 finrl_deepseek 参数，使用默认值")
    except ImportError:
        print("⚠️ pkg.params 不可用，使用默认值")
    return {}
